...
class Brand extends BaseAR
{
    ...
    {
        if (isset($this->img)) {
            unlink(Yii::getAlias('@brandroot/' . $this->img));
        }
    ...
    public function beforeSave($insert)
    {
        ...
        foreach ($tmpImgs as $tmpImg) {
            ...
            copy($imgPath, Yii::getAlias('@brandroot/' . $newName));
            $this->img = $newName;
        }
    ...
    {
        return $this->hasMany(FileToBrand::class, ['brand_id' => 'id'])->andWhere(['type' => FileToBrand::TYPE_BRAND]);
    }
    ...
    public function upload()
    {
        if ($this->validate()) {
            foreach ($this->docFiles as $file) {
                ...
                $res = $file->saveAs(Yii::getAlias('@brandroot/' . $newName));
                if ($res) {
                    $file = new FileToBrand();
                    $file->brand_id = $this->id;
                    $file->name = $newName;
                    $file->type = FileToBrand::TYPE_BRAND;
                    $file->save();
                }
            }
            return true;
