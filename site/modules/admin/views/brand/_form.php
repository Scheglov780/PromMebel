        ...
        <div class="box-body">
            ...
            <?= $form->field($model, 'img')
                ->widget(alexantr\elfinder\InputFile::class, [
                    'clientRoute' => 'brand/input',
                    'filter' => ['image'],
                    'template'      => '<div class="input-group">{input}<span class="input-group-btn">{button}</span></div><img id="prev-img" src="'.Yii::getAlias('@brand/'.$model->img).'" alt="">',
            ...
            <div class="form-group field-brand-docfiles has-success">
                ...
                $dataProvider = new ActiveDataProvider([
                    'query' => \app\models\ar\FileToBrand::find()->where(['brand_id' => $model->id, 'type' =>
                      \app\models\ar\FileToBrand::TYPE_BRAND]),
                              'pagination' => [
              'pageSize' => 100,
            ],
                ]);
                    ...
                    'columns' => [
                      ['class' => 'yii\grid\SerialColumn'],
                        [
                            ...
                            'format' => 'raw',
                            'value' => function($m) {
                                return "<a href='/static/brand/{$m->name}' target='_blank'>{$m->name}</a>";
                            }
                        ],
                        [
                            ...
                            'buttons' => [
                                'delete' => function ($url, $model, $key) {
                                    $url = Url::to(['/admin/brand/delete-file', 'id' => $model->id]);
                                    $options = array_merge([
                                        'title' => 'Delete',
                                        'aria-label' => 'Delete',
                                        'data-pjax' => '0',
...
<script>
    function afterInit() {
        $('#brand-img').on('change', function (e) {
            $('#prev-img').attr('src', $('#brand-img').val());
        });
    }
</script>
