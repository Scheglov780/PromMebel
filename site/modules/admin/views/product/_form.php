...
$properties = \app\models\ar\Property::find()->orderBy("(type='Основа') desc, type asc, -`order` desc")->all();
    ...
    <div class="col-md-6 ">
      <div class="box">
        ...
        <div class="box-body">
            ...
            //ArrayHelper::map(\app\models\ar\Category::getCategoriesWithoutChild(), 'id', 'name'),
              [Product::TYPE_PRODUCT => 'Продукт', Product::TYPE_SERIES => 'Серия'],
              [
          ...
          <div class="form-group">
            <ul id="sortable">
                <?php foreach ($model->getProductImgs()->orderBy('order')->all() as $img) { ?>
                  <li class="ui-state-default" data-id="<?= $img->id ?>"
                      style="background-image: url('<?= Yii::getAlias('@web/static/product/upl/' . $img->name) ?>')">
            ...
            <?= $form->field($model, 'execution')->dropDownList(Product::$executions, ['prompt' => 'Отсутствует']) ?>

            <?= $form->field($model, 'delivery_type')->dropDownList(Product::$deliveryTypes, ['prompt' => 'Отсутствует']
            ) ?>
              ...
              <div class="nav-tabs-custom">
                <ul class="nav nav-tabs">
                    <?php foreach (\app\models\ar\Property::getTypes() as $k => $type) { ?>
                      <li <?php if ($k == 0) echo 'class="active"' ?>><a href="#tab_<?= $type ?>"
                                                                         data-toggle="tab"><?= empty($type) ?
                                'Без группы' : $type ?></a></li>
                    <?php } ?>
                </ul>
                <div class="tab-content" style="overflow-x: hidden; overflow-y: auto; height:100%; max-height:
                1280px;">
                    <?php foreach ($propertiesArray as $type => $properties) { ?>
                      <div class="tab-pane <?php if ($type == \app\models\ar\Property::getTypes()[0]) echo 'active' ?>"
                           id="tab_<?= $type ?>">
                          <?php foreach ($properties as $prop) { ?>
                            <div class="form-group">
                              <label for="<?= "Product[properties][{$prop->id}]" ?>" class="col-sm-4 control-label"><?=
                                  $prop->name ?></label>

                              <div class="col-sm-8">
                                  <?php if (true /* Юзать автокомплит всегда? */ || in_array($prop->name, [
                                      'Срок поставки',
                                      'Доставка',
                                    ])) { ?>
                                    <input type="text" class="form-control" name="Product[properties][<?= $prop->id ?>]"
                                           placeholder="<?= $prop->value_name ?>"
                                           value="<?= @$propValue[$prop->id] ?>"
                                           list="ListProduct[properties][<?= $prop->id ?>]">
                                    <datalist id="ListProduct[properties][<?= $prop->id ?>]">
                                        <?php
                                        $propVals = Yii::$app->db->createCommand(
                                          "select `value` from property_to_product pp 
where pp.property_id = :property_id group by `value`", [':property_id' => $prop->id]
                                        )->queryAll();
                                        if (!empty($propVals)) {
                                        foreach ($propVals
                                        ...
                                        as $propVal) {
                                        ?>
                                      <option label='<?= html_entity_decode($propVal['value']) ?>' value='<?=
                                      $propVal['value'] ?>'>
                                          <?php }
                                          }
                                          ?>
                                      ...
                                      <?php /*                                      echo Html::dropDownList(
                                        "Product[properties][{$prop->id}]",
                                        @$propValue[$prop->id],
                                        ArrayHelper::map($propVals, 'value', 'value'),
                                        [
                                          'prompt' => [
                                            'text'    => 'Не указано',
                                            'options' => [
                                              'value' => null,
                                                //              'class' => 'prompt', 'label' => 'Select'
                                            ],
                                          ],
                                          'class' => 'form-control',
                                          //'options' => [],
                                          'id' => "Product[properties][{$prop->id}]",
                                        ],
                                      ); */
                                      ?>
                                  <?php } else { ?>
        ...
        <div class="box-body">
          <div class="form-group">
              <?= Html::dropDownList(
                'recommendedPackages',
                null,
                ArrayHelper::map($recommendedPackages, 'id', 'name'),
                [
      ...
      <div class="box">
        ...
        <div class="box-body">
          <div class="form-group field-brand-docfiles has-success">
              <?php
              //@todo По аналогии добавить файлы производителя
              $dataProvider = new ActiveDataProvider([
                'query'      => \app\models\ar\FileToBrand::find()->where(
                  ['brand_id' => $model->id, 'type' => \app\models\ar\FileToBrand::TYPE_PRODUCT]
                ),
                'pagination' => [
                  'pageSize' => 100,
                ],
